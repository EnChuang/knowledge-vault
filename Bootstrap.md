...
> **規則層級**：L1 專項。執行見 **kv-moc**。種子見 [[Bootstrap.md]] B4。

---

## 本檔目錄

§1 定位 · §2 四段結構 · §3 各章節 · §4 M1–M10 · §5 維護 · §6 生長 · §8 待延伸
...
- 整理筆記＝白皮書；80% 覆蓋；讀者自測 10–20 題
- **Ebook 蒸餾預設**：§12（公式＋註解＋比喻、每章 10 題＋答案區、Exam 閘門）；用戶另指定則例外
- 講解整理重點：比喻釐清（先喻後理，見 §5）
- 檔名＝`title`＝wikilink；禁泛稱；Ebook 章 `{章號} · {章標題}.md`
- `tags`：`領域/子領域`；**禁止空格**（多詞用連字號）
- §10 更名全庫 grep；§11 健檢見 kv-audit
- 分段整理：只改指定段
...
> **規則層級**：L1 專項。執行見 **kv-source**。種子見 [[Bootstrap.md]] B6。
...
| # | 檢查 | 通過條件 |
|---|------|----------|
| V1 | 檔案存在 | B1 所列 + `MEMORY.md` + `skills/kv-*/SKILL.md`（14 包） |
...
| **MOC_rules** | 四段；禁 mermaid；M1–M10；生長；待延伸 |
| **Project_rules** | 白皮書；自測；§9 標籤；§10 更名；§11 |
| **Source_rules** | 三子夾；連結；引讀五步；G3/G4 |
