...
把 **Obsidian** 當你的檔案室，把 **Agent 執行環境** 當會遵守規矩的助理——  
...
| **讀得完的筆記** | **科普掃讀**（30 秒帶走、分鏡表卡、收束）；**中文術語完整**（不因英譯縮成半截詞）；可自測；Ebook 另有章節格式 |
...
| 「這章寫完要能複習」 | 一般知識筆記可帶讀者自測；使用指南改用常見問題／操作檢查表；電子書有固定章節格式 |
...
- 能讀庫根 `AGENTS.md` 與 `.agent/skills/` 的相容 Agent 執行環境  
- 本倉庫（clone 或下載 ZIP）  
- 一個空資料夾當「庫根」
...
建庫完成後，AI 還會幫你長出 `*_rules.md`、`Source/` 子目錄，以及 `.agent/skills/`（執行用鏡像）。
...
**一定要 Agent 嗎？**  
這套 Skill 路徑是依 Agent 執行環境 設計的。其他 Agent 若能讀同一套檔案可以試，但要自己驗。
...
**Skill 要裝到使用者目錄嗎？**  
**不要。** 建庫時放在**這個知識庫資料夾**裡的 `.agent/skills/` 即可。

**中文為什麼有時看起來像半截詞？**  
規則要求**中文術語語意完整**（例如寫「錨點」而不是只寫「錨」），避免英譯省略讓讀者靠英文腦補。寫筆記與改開箱文案時都適用；給別人讀的文字更嚴。
